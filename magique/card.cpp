...
std::set<card::color> card::color_identities_from_array(const std::set<std::string> arr)
{
    ...
    for (auto color : arr)
    {
        ...
        if (color == "w")
        { colors.emplace(card::color::white); }
        else if (color == "u")
        { colors.emplace(card::color::blue); }
        else if (color == "b")
        { colors.emplace(card::color::black); }
        else if (color == "r")
        { colors.emplace(card::color::red); }
        else if (color == "g")
        { colors.emplace(card::color::green); }
        else if (color == "u") colors.emplace(card::color::colorless);
    }
...
card::types_from_array(const std::set<std::string> &types, const std::set<std::string> &supertypes)
{
    ...
    for (const auto &type : types)
    {
        ...
        if (type_str == "land")
        {
            ...
            if (basic_land)
            {
                types_arr.emplace(card::type::basic_land);
            }

            types_arr.emplace(card::type::land); // basic lands are also lands
        }
        else if (type_str == "creature")
        { types_arr.emplace(card::type::creature); }
        else if (type_str == "artifact")
        { types_arr.emplace(card::type::artifact); }
        else if (type_str == "enchantment")
        { types_arr.emplace(card::type::enchantment); }
        else if (type_str == "planeswalker")
        { types_arr.emplace(card::type::planeswalker); }
        else if (type_str == "instant")
        { types_arr.emplace(card::type::instant); }
        else if (type_str == "sorcery") types_arr.emplace(card::type::sorcery);
